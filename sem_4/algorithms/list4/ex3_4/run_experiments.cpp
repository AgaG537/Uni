...
AggregatedMetrics run_tests_and_aggregate(int n_val, bool ascending_insert, int num_tests) {
    ...
    for (int test = 0; test < num_tests; ++test) {
        ...
        for (int key : insert_sequence) {
            ...
            rbTree.insert(key);
            all_comparisons.push_back(rbTree.comparisons);
            all_pointer_ops.push_back(rbTree.pointer_operations);
            all_heights.push_back(rbTree.height());
        }

        for (int key : delete_sequence) {
            rbTree.reset_metrics();
            rbTree.delete_value(key);
            all_comparisons.push_back(rbTree.comparisons);
            all_pointer_ops.push_back(rbTree.pointer_operations);
            all_heights.push_back(rbTree.height());
        }
