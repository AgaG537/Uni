...
// --- Shared Variables for Dekker's Algorithm (Atomic) ---
var (
	// 0: interested, 1: not interested
	C    = make([]int32, NrOfProcesses) // Automatically 0 initialized
	Turn = int32(0)                     // 0 for process 0, 1 for process 1
)
...
// --- Main Program ---
func main() {
	// Initialize C array flags to 1 (not interested)
	for i := range C {
		C[i] = 1
	}
