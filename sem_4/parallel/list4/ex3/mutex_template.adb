...
-- Processes 
  Nr_Of_Reading_Processes : constant Integer := 10;
  Nr_Of_Writing_Processes : constant Integer := 5;
  Nr_Of_Processes : constant Integer := Nr_Of_Reading_Processes + Nr_Of_Writing_Processes;
...
-- Types, procedures and functions
  ...
  task type Reading_Process_Task_Type is	
    entry Init(Id: Integer; Seed: Integer);
    entry Start;
  end Reading_Process_Task_Type;	

  task body Reading_Process_Task_Type is
    G : Generator;
...
--    for Step in 0 .. Nr_of_Steps loop
  ...
  task type Writing_Process_Task_Type is	
    entry Init(Id: Integer; Seed: Integer);
    entry Start;
  end Writing_Process_Task_Type;	

  task body Writing_Process_Task_Type is
    G : Generator;
    Process : Process_Type;
    Time_Stamp : Duration;
    Nr_of_Steps: Integer;
...
-- local for main task
  ...
  Reading_Process_Tasks: array (0 .. Nr_Of_Reading_Processes-1) of Reading_Process_Task_Type;
  Writing_Process_Tasks: array (0 .. Nr_Of_Writing_Processes-1) of Writing_Process_Task_Type; 

begin 
  -- init tarvelers tasks
  for I in Reading_Process_Tasks'Range loop
    Reading_Process_Tasks(I).Init(I, Seeds(I+1));   -- `Seeds(I+1)` is ugly :-(
  end loop;

  for I in Writing_Process_Tasks'Range loop
    Writing_Process_Tasks(I).Init(I + Nr_Of_Reading_Processes, Seeds(I+ 1 + Nr_Of_Reading_Processes));   -- `Seeds(I+1)` is ugly :-(
  end loop;

  -- start tarvelers tasks
  for I in Reading_Process_Tasks'Range loop
    Reading_Process_Tasks(I).Start;
  end loop;

  for I in Writing_Process_Tasks'Range loop
    Writing_Process_Tasks(I).Start;
  end loop;
